...
| --- | --- | --- |
| [side-dock-elevation.html](side-dock-elevation.html) | Side dock (130×360) | The dock shipped as a 240 px opaque slab with an upside-down wordmark and nothing on it outside a dictation. Before/at-rest/recording elevations at 1:1, the row arithmetic, the placement diagram for the 24 px work-area inset, a change ledger, and the two defects only the running window showed. **Built** — geometry and placement measured against the live app. **The geometry it draws is superseded**: it depicts a 130-wide card with five rows (20 / 86 / 152 / 16 / 28), and the dock has since narrowed to 62 and gained a sixth row for the engine indicator (20 / 104 / 14 / 112 / 16 / 28, 2026-08-27). Superseded again on 2026-08-28, and this time in the order as well as the sizes: the window grew to 400 to pay for an action button present in every state, the engine row moved *below* the meter, and the clock row became a status row that also carries the outcome mark — 20 / 104 / 152 / 14 / 16 / 28. Superseded a third time on 2026-09-22: the status row folded into the button and a stray narrow-width padding rule stopped reaching the card; on 2026-09-23 the engine row settled directly above the button — 20 / 90 / 178 / 14 / 36. The argument it makes still holds; the numbers on it were true when drawn and are not now. |
| [settings-workspace-refresh.html](settings-workspace-refresh.html) | Settings workspace (six pages) | A standalone, interactive proposal that keeps the live workspace's accessibility and layout contracts while introducing a compact product header, stronger page and section hierarchy, reusable setting rows, readable runtime facts, explicit status treatments, and a fixed page grid at narrow widths. Light/dark themes and keyboard page navigation are included. **Built in v1.9.0** — the production workspace follows this direction while preserving its real controls, failure disclosure, polling, and backend behavior. **Superseded 2026-09-23** by settings-simplified.html below. |
| [settings-simplified.html](settings-simplified.html) | Settings workspace (five pages) | The workspace uses six different presentations for a setting, two save models, text-only sections, model cards for an install the app does not perform, and a tinted multi-hue palette. Proposes five pages (General, Microphone, Transcription, History, Advanced) built from one row pattern with instant apply, one neutral palette with a single accent, a map from every current control to its new home, the invariants the rework must keep, and the owner decisions it needs. Interactive, light/dark, with an accent comparison. **Built** (2026-09-23) — the production workspace follows it: five pages, one row pattern, the neutral palette with the blue accent, and every owner decision in its section 5 taken as proposed. The drawing itself still says "Proposal"; the numbers in it were true when drawn. |
